...
import model.domain.Personal;
	...
	@Service
	public class BoletasPagosManager {
		...
		public List<Mpagos> listarAdicionales(int xcodper){			
			String xsql="   select m.codpag,m.fecha,m.monto,m.login,m.obs  "
						+"	from mpagos m, bol_adicional b  "
						+"	where b.codper=? and b.codpag=m.codpag and m.estado=1  ";			
		...
		public List<Mpagos> listaMaestroPagos(int xest1, int xest2, Date xfini, Date xffin){			
			String xsql="   select DISTINCT m.codpag,m.estado,m.contador,m.fecha,m.concepto,m.monto,p.nombre,p.ap,p.am, f.nrofac,f.codigocontrol  "
						+"	from mpagos m, dpagos d, boletas b, estado e, personal p, factura f  "
						...
						+"	      (d.mes=b.mes)and(d.anio=b.anio)and(b.codestado=e.codestado)and(e.codper=p.codper)and(m.codpag=f.codpag)and(m.facturaman=0) "
		...
		public List<Boletas> listaDetalle_Boletas_pagados(String xcodpag){			
			String xsql="   select d.codpag,d.codestado,d.mes,d.anio,d.monto "
						+"	from dpagos d "
						+"	where (d.codpag like ?) "
		...
		public String setAddMpagos(Date xfecha, String xlogin, String xobs,double xtotal,String xcliente,String xnit,
				                   int xdia,int xmes,int xanio,String xmontotext,String xestsoc1,String xdetalle,float xmonadicional,int xmisFilas){
			String xsql="select adicionarMpagos(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			return this.jdbcTemplate.queryForObject(xsql, String.class,new Object[] {xfecha,xlogin, xobs, xtotal, xcliente,xnit,xdia,xmes,xanio,xmontotext,xestsoc1,xdetalle,xmonadicional,xmisFilas});
		}
		
		public String setDelMpagos(String xcodpag, int xnroFac){
			String xsql="select eliminarMpagos(?,?)";
			return this.jdbcTemplate.queryForObject(xsql, String.class,new Object[] {xcodpag,xnroFac});
		}
		
		public String setAddDetallepagos(String xcodpag,int xcodestado,int xmes,int xanio, float xmonto, String xcodControl, int contador, int xcontador){
			String xsql="select adicionarDetallePagos(?,?,?,?,?,?,?,?)";
			return this.jdbcTemplate.queryForObject(xsql, String.class,new Object[] {xcodpag,xcodestado,xmes,xanio,xmonto,xcodControl,contador,xcontador});
		}
