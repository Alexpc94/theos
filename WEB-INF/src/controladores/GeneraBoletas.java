...
public class GeneraBoletas {
	...
	@RequestMapping({"generarBoletas100.html"})
	public String generarBoletas100(Model model,HttpServletRequest request)  throws IOException  {
		...
		if (xusuario == null){
			...
			VarGlobales var=new VarGlobales("100",crypt.decrypt(cd)); //params: (proceso, menu)
			...
			model.addAttribute("opAdd", crypt.encrypt("addGenerarBoletas"));
		}		
		model.addAttribute("file1", comodoTpl+"/generaBoletas/generaboletasmon.vm");	
		return "marco";
	}
	
	
	
	@RequestMapping({"generarBoletasServices.html"})
	public String generarBoletasServices(Model model,HttpServletRequest request,HttpServletResponse response)  throws IOException  {
		...
		if (xusuario == null){
			...
			if (op.equals("addGenerarBoletas")){	
				String xmes=request.getParameter("xmes");
				String xanio=request.getParameter("xanio");
				
				System.out.println(" xmes ="+xmes+" xanio="+xanio);
...
/************************     REPORTES   ************************************************************************************** */
	  ...
	  @RequestMapping("generaBoletasReportes01.html")
